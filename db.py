...
def create_user(telegram_id: int, sub_id: str, client_uuid: str, expires_at: datetime.datetime = None, trial_used: int = 0, notified_3_days: int = 0):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    created_at = datetime.datetime.now()
    try:
        cursor.execute(
            "INSERT INTO users (telegram_id, sub_id, created_at, expires_at, client_uuid, trial_used, notified_3_days) VALUES (?, ?, ?, ?, ?, ?, ?)",
            (telegram_id, sub_id, created_at, expires_at, client_uuid, trial_used, notified_3_days)
        )
        conn.commit()
...
def create_pending_payment(payment_id: str, telegram_id: int, plan_duration_days: int):
    conn = sqlite3.connect(DB_FILE)
    cursor = conn.cursor()
    created_at = datetime.datetime.now()
    cursor.execute(
        "INSERT INTO pending_payments (payment_id, telegram_id, plan_duration_days, created_at) VALUES (?, ?, ?, ?)",
        (payment_id, telegram_id, plan_duration_days, created_at)
    )
    conn.commit()
    conn.close()
